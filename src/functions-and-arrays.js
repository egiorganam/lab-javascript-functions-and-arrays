...
const averageWordLength = arr => {
  ...
  arr.forEach(e => arrTwo.push(e.length))

  arrTwo.forEach(e => sum += e)
...
const doesWordExist = (arr, find) => {
  if(arr.length === 0) return null
  
  let qpr = undefined
  arr.forEach(e => e === find ? qpr = true : qpr = false)
